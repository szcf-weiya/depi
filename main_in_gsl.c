...
# pragma omp for schedule(dynamic)
  for (int i = 0; i < MAX_COL/500; i++)
    {
      gsl_matrix_get_col(x1, G, i);
      //      gsl_matrix_get_col(x3, G, i);//more fast
      //gsl_vector_memcpy(x3, x1); // spend a lot time 33
      ...
      for (int j = i+1; j < MAX_COL; j++)
	{
	  ...
	  if (abs(res_corr) < MAX_PAIR_CORR)
		...
		//output << G_colname.at(i) << "\t" << G_colname.at(j) << "\t" << pvalue[0] << "\t" << pvalue[1] << "\t" << pvalue[2] <<endl;
